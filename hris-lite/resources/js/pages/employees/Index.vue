...
const exportUrl = computed(() => {
    ...
    if (props.filters.search) params.set('search', String(props.filters.search));
    if (branchFilter.value) params.set('branch_id', branchFilter.value);
    ...
    <div class="flex flex-col gap-6">
        ...
        >
            <template #toolbar>
                ...
                <Select v-model="departmentModel">
                    ...
                    <SelectContent>
                        ...
                        <SelectItem v-for="d in availableDepartments" :key="d.id" :value="String(d.id)">{{ d.name }}</SelectItem>
                    </SelectContent>
                </Select>
            ...
            <template #cell-name="{ row }">
                <Link :href="`/employees/${row.id}`" class="flex items-center gap-2 hover:underline">
                    <img v-if="row.photo_url" :src="row.photo_url" alt="" class="size-7 rounded-full object-cover" />
            ...
            <template #cell-actions="{ row }">
                <div class="flex justify-end gap-1">
                    <Button variant="ghost" size="icon-sm" as-child>
                        <Link :href="`/employees/${row.id}`" title="View 201 file">
                            <Eye class="size-4" />
                        </Link>
                    </Button>
                    <Button variant="ghost" size="icon-sm" as-child>
                    ...
                    >
                        <UserRoundX v-if="isActive(row)" class="size-4 text-destructive" />
                        <UserRoundCheck v-else class="size-4 text-emerald-600" />
                    </Button>
                </div>
            </template>
        </DataTable>
